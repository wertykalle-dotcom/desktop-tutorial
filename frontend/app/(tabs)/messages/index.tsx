...
type MessagesResponse = {
  threads: (ThreadItem & { unread_count?: number; avatar_url?: string | null; last_sender_username?: string | null; last_read_at?: string | null; last_message_delivered_at?: string | null; last_message_read_at?: string | null; last_message_state?: 'sending' | 'delivered' | 'read' | null; is_typing?: boolean; is_online?: boolean })[];
  unread_count: number;
};
...
export default function MessagesScreen() {
  ...
  const loadThreads = useCallback(async () => {
    setLoading(true);
    const response = await apiFetch('/messages?limit=10', {}, { requireAuth: true });
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {threads.length === 0 ? (
        ...
        threads.map((thread) => (
          <Pressable key={thread.id} onPress={() => router.push(`/messages/${thread.id}` as never)} style={styles.card}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.statusMeta, isRTL && styles.textRight]}>
                {thread.last_message_state === 'read' ? t('messageRead') : thread.last_message_state === 'delivered' ? t('messageDelivered') : thread.last_message_state === 'sending' ? t('messageSending') : thread.unread_count ? t('messagesUnreadSince') : t('messageDelivered')}
              </Text>
